...
def _write_database_job(mode, state, percent, phase, message=""):
    ...
    phase_text = str(phase)[:100]
    message_text = str(message)[:1200]
    previous = _read_json(DATABASE_JOB_FILE)
    logs = previous.get("logs", [])
    if not isinstance(logs, list) or previous.get("mode") != mode or (progress <= 1 and phase_text == "Bắt đầu"):
        logs = []
    entry = {
    ...
    # Chỉ thêm khi bước, trạng thái hoặc phần trăm đổi để log ngắn và dễ đọc.
    if any(last.get(key) != entry[key] for key in ("percent", "phase", "state")):
        logs.append(entry)
...
*{box-sizing:border-box}body{margin:0;min-height:100vh;padding:28px;background:radial-gradient(circle at 70% 0,rgba(201,164,95,.15),transparent 32rem),#090705;color:#e8dfd1;font:14px/1.5 Inter,system-ui,sans-serif}.wizard{width:min(820px,100%);margin:auto}.brand{color:#f0d9aa;font:700 25px Georgia,serif;margin-bottom:20px}.steps{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:16px}.step{padding:10px 12px;border:1px solid #443426;border-radius:9px;color:#958879;background:#110d0a}.step.done{color:#58d8b8;border-color:#215c4f}.step.active{color:#f2d58f;border-color:#a67e38;background:#1b140c}.card{padding:24px;border:1px solid #443426;border-radius:14px;background:#15100c;box-shadow:0 24px 70px rgba(0,0,0,.45)}h1{font:750 25px Inter,ui-sans-serif,system-ui,-apple-system,"Segoe UI",sans-serif;margin:0 0 6px;color:#f0d9aa}p{color:#a99b8a}.password-toolbar{display:flex;justify-content:flex-end;margin:0 0 8px}.password-toolbar button{padding:7px 11px;font-size:12px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}.db{padding:15px;border:1px solid #3a2e24;border-radius:10px;background:#0e0b09}.db h2{font-size:16px;margin:0 0 5px}.db p{font-size:12px;margin:0 0 12px}label{display:block;color:#b8aa99;font-size:12px;margin-top:9px}input,button{font:inherit;border-radius:8px}input{width:100%;padding:10px 11px;margin-top:4px;border:1px solid #4a392b;background:#080706;color:#fff}input:focus{outline:0;border-color:#c9a45f;box-shadow:0 0 0 3px rgba(201,164,95,.12)}button{border:1px solid #936d31;background:#9d7838;color:#fff8e9;padding:10px 14px;font-weight:750;cursor:pointer}.secondary{background:#252015;border-color:#50412f;color:#d8c9b4}.tools{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-top:17px}.hint{font-size:12px}.progress{margin-top:18px;padding:15px;border:1px solid #4a392b;border-radius:10px;background:#090706}.bar{height:11px;overflow:hidden;border-radius:20px;background:#211b15}.bar>i{display:block;height:100%;width:0;background:linear-gradient(90deg,#9b7331,#e0bb65);transition:width .3s}.progress-head{display:flex;justify-content:space-between;margin-bottom:8px}.message{color:#a99b8a;margin:8px 0 0;white-space:pre-wrap}.setup-log{margin-top:12px;padding-top:11px;border-top:1px solid #33271e}.setup-log-head{display:flex;justify-content:space-between;gap:10px;margin-bottom:7px;color:#d7c7b3;font-size:12px}.setup-log-head span{color:#7f7468}.setup-log-lines{max-height:145px;overflow:auto;padding:7px 9px;border:1px solid #2f271f;border-radius:7px;background:#050403;color:#bfb2a3;font:11px/1.45 ui-monospace,SFMono-Regular,Consolas,monospace}.setup-log-line{display:grid;grid-template-columns:56px 38px minmax(0,1fr);gap:7px;padding:3px 0;border-bottom:1px dashed #211b16}.setup-log-line:last-child{border-bottom:0}.setup-log-time{color:#776d62}.setup-log-percent{color:#d8b663;text-align:right}.setup-log-text{overflow-wrap:anywhere}.setup-log-line.done .setup-log-text{color:#58d8b8}.setup-log-line.error .setup-log-text{color:#ef8078}.setup-log-empty{color:#71675d;padding:3px 0}.progress.error{border-color:#8b3733}.progress.error .message{color:#ef8078}.progress.done{border-color:#216452}.progress.done .message{color:#58d8b8}.enter{display:none;margin-top:12px;text-decoration:none;text-align:center;background:#187d69;border:1px solid #2ab89a;color:#fff;padding:11px;border-radius:8px;font-weight:800}.enter.show{display:block}@media(max-width:700px){body{padding:14px}.steps,.grid{grid-template-columns:1fr}.card{padding:17px}.tools{align-items:stretch;flex-direction:column}.tools button{width:100%}.setup-log-line{grid-template-columns:50px 34px minmax(0,1fr)}}
</style></head><body><main class="wizard"><div class="brand">⚔ JXNative</div><div class="steps"><div class="step done">✓ 1. Đăng nhập Admin</div><div class="step done">✓ 2. Đổi mật khẩu Admin</div><div class="step active">3. Khởi tạo database</div></div><section class="card"><h1>Thiết lập MySQL và MSSQL lần đầu</h1><p>Chỉ thực hiện một lần. Game server chưa tự khởi động sau bước này.</p>{% if has_credentials %}<p class="hint">Đã lưu mật khẩu từ lần chạy trước. Nếu trước đó bị lỗi, để trống tất cả ô và bấm <b>Thử lại thiết lập</b>.</p>{% endif %}<form id="setupForm" method="post"><input type="hidden" name="csrf_token" value="{{csrf}}"><div class="password-toolbar"><button type="button" class="secondary" id="toggleGeneratedPasswords">◉ Hiện mật khẩu</button></div><div class="grid"><div class="db"><h2>MySQL — dữ liệu đăng nhập</h2><p>Mật khẩu tài khoản root dùng bởi Web, backup và Goddess.</p><label>Mật khẩu mới<input type="password" name="mysql_password" autocomplete="new-password" {{'' if has_credentials else 'required'}}></label><label>Nhập lại<input type="password" name="mysql_confirm" autocomplete="new-password" {{'' if has_credentials else 'required'}}></label></div><div class="db"><h2>MSSQL — dữ liệu nhân vật</h2><p>Mật khẩu tài khoản sa dùng bởi Web, backup và PaySys native.</p><label>Mật khẩu mới<input type="password" name="mssql_password" autocomplete="new-password" {{'' if has_credentials else 'required'}}></label><label>Nhập lại<input type="password" name="mssql_confirm" autocomplete="new-password" {{'' if has_credentials else 'required'}}></label></div></div><div class="tools"><span class="hint">12–20 ký tự; có hoa, thường, số và @ _ ! . hoặc -</span><div><button type="button" class="secondary" id="generatePasswords">Tạo 2 mật khẩu mạnh</button> <button id="startSetup">{{'Thử lại thiết lập' if has_credentials else 'Bắt đầu cài database'}}</button></div></div></form><div class="progress" id="progressBox" hidden><div class="progress-head"><b id="phase">Đang chuẩn bị</b><span id="percent">0%</span></div><div class="bar"><i id="bar"></i></div><p class="message" id="message"></p><div class="setup-log"><div class="setup-log-head"><b>Nhật ký cài đặt</b><span>40 mốc gần nhất</span></div><div class="setup-log-lines" id="setupLog"><div class="setup-log-empty">Chưa có nhật ký.</div></div></div><a class="enter" id="enter" href="{{url_for('dashboard')}}">Vào bảng điều khiển</a></div></section></main><script>
(function(){
const form=document.getElementById('setupForm'),box=document.getElementById('progressBox'),phase=document.getElementById('phase'),percent=document.getElementById('percent'),bar=document.getElementById('bar'),message=document.getElementById('message'),setupLog=document.getElementById('setupLog'),start=document.getElementById('startSetup'),enter=document.getElementById('enter');
function makePassword(){const lower='abcdefghijkmnopqrstuvwxyz',upper='ABCDEFGHJKLMNPQRSTUVWXYZ',digits='23456789',symbols='@_!-',all=lower+upper+digits+symbols,a=[lower,upper,digits,symbols].map(s=>s[crypto.getRandomValues(new Uint32Array(1))[0]%s.length]);while(a.length<16)a.push(all[crypto.getRandomValues(new Uint32Array(1))[0]%all.length]);for(let i=a.length-1;i>0;i--){const j=crypto.getRandomValues(new Uint32Array(1))[0]%(i+1);[a[i],a[j]]=[a[j],a[i]]}return a.join('')}
function renderLog(entries){setupLog.replaceChildren();const rows=Array.isArray(entries)?entries.slice(-40):[];if(!rows.length){const empty=document.createElement('div');empty.className='setup-log-empty';empty.textContent='Chưa có nhật ký.';setupLog.append(empty);return}rows.forEach(item=>{const row=document.createElement('div');row.className='setup-log-line '+(item.state||'');const when=document.createElement('span'),pct=document.createElement('span'),detail=document.createElement('span');when.className='setup-log-time';pct.className='setup-log-percent';detail.className='setup-log-text';when.textContent=item.time||'--:--:--';pct.textContent=String(item.percent??0)+'%';detail.textContent=(item.phase||'Đang xử lý')+(item.message?' — '+item.message:'');row.append(when,pct,detail);setupLog.append(row)});setupLog.scrollTop=setupLog.scrollHeight}
document.getElementById('generatePasswords').addEventListener('click',()=>{const one=makePassword(),two=makePassword();form.mysql_password.value=form.mysql_confirm.value=one;form.mssql_password.value=form.mssql_confirm.value=two});
document.getElementById('toggleGeneratedPasswords').addEventListener('click',function(){const visible=form.mysql_password.type==='text';[form.mysql_password,form.mysql_confirm,form.mssql_password,form.mssql_confirm].forEach(input=>input.type=visible?'password':'text');this.textContent=visible?'◉ Hiện mật khẩu':'◉ Ẩn mật khẩu'});
async function poll(){try{const response=await fetch('{{url_for("manager_ext.database_job_status")}}',{cache:'no-store'}),data=await response.json();box.hidden=false;box.className='progress '+(data.state||'');phase.textContent=data.phase||'Đang xử lý';percent.textContent=(data.percent||0)+'%';bar.style.width=(data.percent||0)+'%';message.textContent=data.message||'';renderLog(data.logs);if(data.state==='done'){enter.classList.add('show');start.disabled=false;return}if(data.state==='error'){start.disabled=false;return}setTimeout(poll,1000)}catch(error){message.textContent='Mất kết nối tạm thời, đang thử lại…';setTimeout(poll,1800)}}
form.addEventListener('submit',async event=>{event.preventDefault();start.disabled=true;enter.classList.remove('show');box.hidden=false;box.className='progress';phase.textContent='Đang gửi yêu cầu';percent.textContent='0%';bar.style.width='0%';message.textContent='';renderLog([{time:new Date().toLocaleTimeString('vi-VN',{hour12:false}),percent:0,phase:'Đang gửi yêu cầu',state:'running'}]);try{const response=await fetch(form.action||location.href,{method:'POST',body:new FormData(form),headers:{'X-Requested-With':'XMLHttpRequest'}}),data=await response.json();if(!response.ok)throw new Error(data.error||'Không bắt đầu được thiết lập');poll()}catch(error){box.className='progress error';phase.textContent='Chưa thể bắt đầu';message.textContent=error.message;renderLog([{time:new Date().toLocaleTimeString('vi-VN',{hour12:false}),percent:0,phase:'Chưa thể bắt đầu',message:error.message,state:'error'}]);start.disabled=false}});{% if job_available %}poll();{% endif %}
})();
</script></body></html>
"""
